...
ntax = 117
times =  c(0.0,   20.0,   40.0,  60.0,   80.0,  105.0)
lambda = c(0.13,   0.04,   0.04,  0.07,   0.015,  0.04)
mu =     c(0.018,  0.0075, 0.004, 0.01,   0.0015, 0.004)
psi =    c(0.0003, 0.006,  0.009, 0.0055, 0.008,  0.01)
numsim = 100

# fbd_trees <- sim.fbd.rateshift.taxa(ntax, numsim, lambda, mu, psi, times, complete=FALSE)
fbd_trees <- list()
i <- 1
while(i <= numsim) {
    maxage <- 1
    while(maxage < 8) {
        print(paste("attempt at sim ", i, sep=""))
        fbd_tree <- sim.fbd.rateshift.taxa(ntax, 1, lambda, mu, psi, times, complete=FALSE)[[1]]
        maxage <- max(node.depth.edgelength(fbd_tree))
    }
    fbd_trees[[i]] <- fbd_tree
    i <- i+1
...
for (i in 1:length(fbd_trees)) {
    write.tree(fbd_trees[[i]], paste("tree_skyline_", i, ".tre", sep=""))
}

for (i in 1:length(fbd_trees)) {
    write.tree(extanttrees[[i]], paste("extanttree_skyline_", i, ".tre", sep=""))
}


for (i in 1:length(fbd_trees)) {
    write.csv(treeinfo[[i]], paste("treeinfo_skyline_", i, ".csv", sep=""))
}


for (i in 1:length(fbd_trees)) {
    write.table(fossils[[i]], file=paste("fossils_skyline_", i, ".tsv", sep=""), quote=FALSE, sep='\t', row.names=FALSE)
}

for (i in 1:length(fbd_trees)) {
    write.table(taxa[[i]], file=paste("taxa_skyline_", i, ".tsv", sep=""), quote=FALSE, sep='\t', row.names=FALSE)
}


for (i in 1:length(fbd_trees)) {
    write.table(ingroups[[i]], file=paste("ingroups_skyline_", i, ".tsv", sep=""), quote=FALSE, sep='\t', row.names=FALSE)
}
